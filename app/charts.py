...
async def render_google_finance_chart(
    ...
    if prev_close is None or price is None or change_percent is None:
        try:
            ...
            if currency is None:
                currency = "USD" if quote.asset_class in {"stock", "index", "commodity", "metal", "forex", "market"} else None
        except Exception:
            pass
    ...
    fig, ax = plt.subplots(figsize=(12.8, 8.15), dpi=160, facecolor="#202124")
    ...
    fig.subplots_adjust(left=0.035, right=0.90, top=0.79, bottom=0.30)
    fig.text(0.055, 0.205, "Open", ha="left", va="center", fontsize=8.5, color="#9aa0a6")
    fig.text(0.20, 0.205, stats["Open"], ha="left", va="center", fontsize=9.2, fontweight="bold", color="#f8fafc")
    fig.text(0.51, 0.205, "High", ha="left", va="center", fontsize=8.5, color="#9aa0a6")
    fig.text(0.66, 0.205, stats["High"], ha="left", va="center", fontsize=9.2, fontweight="bold", color="#f8fafc")
    fig.text(0.055, 0.135, "Low", ha="left", va="center", fontsize=8.5, color="#9aa0a6")
    fig.text(0.20, 0.135, stats["Low"], ha="left", va="center", fontsize=9.2, fontweight="bold", color="#f8fafc")
    fig.text(0.51, 0.135, "Volume", ha="left", va="center", fontsize=8.5, color="#9aa0a6")
    fig.text(0.66, 0.135, stats["Volume"], ha="left", va="center", fontsize=9.2, fontweight="bold", color="#f8fafc")
...
async def render_chart(
    ...
    *,
    ...
    if not advanced:
        return await render_google_finance_chart(df, symbol, timeframe, prev_close=prev_close, price=price, currency=currency, change_percent=change_percent)
