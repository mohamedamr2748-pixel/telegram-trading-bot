...
def _market_tone(quotes: list[object]) -> str:
    moves = [
        ...
        for q in quotes
        if hasattr(q, "change_percent") and q.change_percent is not None
    ]
    if not moves:
        return "🟡 Mixed"
...
async def _snapshot(market: MarketService) -> tuple[str, InlineKeyboardMarkup]:
    ...
    us_status = _session_label(
        getattr(quotes[0], "market_status", None)
        if not isinstance(quotes[0], Exception)
        else None
    )
    lines = [
        "🌍 <b>MARKET OVERVIEW</b>",
    ...
    for symbol, quote in zip(SYMBOLS[:3], quotes[:3]):
        if isinstance(quote, Exception):
            lines.append(f"{LABELS[symbol]}  <code>n/a</code>  ⚪ unavailable")
        else:
            lines.append(
                f"{LABELS[symbol]}  <code>{_price(symbol, quote.price)}</code>  "
                f"{_badge(quote.change_percent)}"
            )

    lines.extend(["", f"US Market: {us_status}", "", "🪙 <b>CRYPTO</b>", ""])
    btc = quotes[3]
    if isinstance(btc, Exception):
        lines.append("Bitcoin  <code>n/a</code>  ⚪ unavailable")
    else:
        lines.append(
            f"Bitcoin  <code>{_price('BTC-USD', btc.price)}</code>  "
            f"{_badge(btc.change_percent)}"
        )

    lines.extend(["", "🥇 <b>COMMODITIES</b>", ""])
    gold = quotes[4]
    if isinstance(gold, Exception):
        lines.append("Gold  <code>n/a</code>  ⚪ unavailable")
    else:
        lines.append(
            f"Gold  <code>{_price('GC=F', gold.price)}</code>  "
            f"{_badge(gold.change_percent)}"
        )
    ...
    lines.extend(
        [
            "",
            "━━━━━━━━━━━━━━━━",
            f"📊 Market tone: <b>{_market_tone(valid_quotes)}</b>",
...
async def _market_callback(callback: CallbackQuery) -> None:
    ...
    if kind == "movers":
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="⬅️ Market Overview", callback_data="marketv2:refresh")]
            ]
        )
        try:
